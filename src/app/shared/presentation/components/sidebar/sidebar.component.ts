  ...
  ngOnInit() {
    ...
    this.userService.getUserById(numericId).subscribe({
      next: (userData) => {
        ...
        // Asignar la ubicación directamente desde el backend (formato: "Región, País")
    ...
    // Suscribirse a cambios de ubicación desde el perfil
    this.locationSubscription = this.userEventsService.userLocationChanged$.subscribe({
      next: (payload) => {
        this.userLocation.set(payload.location.trim());
